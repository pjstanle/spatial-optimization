...
def spacing_obj(x):
    ...
    if nturbs == 0:
        ...
        if spacing_con < 0.0:
            ...
            # new_spacing = calc_spacing(new_x,new_y)
            return -sum(spacing)
...
if __name__=="__main__":
    ...
    progress_filename = "aep_spacing/turbine%s_setback%s_mesh_run%s.txt"%(turbine,setback_mult,run_number)
    ...
    for i in range(iters):
        ...
        for k in range(nstarts):
            ...
            ga.objective_function = spacing_obj 
            ...
            ga.optimize_ga(crossover="random",initialize="limit",initialize_custom=initialize_custom,print_progress=False,save_progress=False) # run the optimization. I have flags set to print progress and save the results every 1 iterations.
        ...
        if best_sol >= 0.0:
            turbine_x = np.append(turbine_x,best_x)
            turbine_y = np.append(turbine_y,best_y)
        
        turbs_per_zone[i] = len(best_x)
        
        plant.modify_coordinates(turbine_x,turbine_y)
        plant.simulate(1)
        aep = plant.annual_energy_kw()/scale

        file = open('%s'%progress_filename, 'w')
        file.write('%s'%(aep) + '\n' + '\n')
        file.write("AEP: " + '%s'%(aep) + '\n' + '\n')
